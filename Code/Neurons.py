...
class CooldownNeuron(nn.Module):
    ...
    def get_initial_state(self, batch_size):
        return self.initial_mem.expand([batch_size, self.size])
...
class LIFNeuron(nn.Module):
    ...
    def get_initial_state(self, batch_size):
        h = [self.initial_mem.expand([batch_size, self.size])]
        if self.config['ALPHA'] > 0:
            h.append(self.initial_syn.expand([batch_size, self.size]))
        return tuple(h)

    def get_initial_output(self, batch_size):
        return torch.zeros([batch_size, self.size])
