...
class FlipFlopNFn(torch.autograd.Function):

    @staticmethod
    def forward(ctx, on_signal, off_signal, state, decay, counter_dummy):
        ctx.save_for_backward(decay)
        out = ((state + on_signal - off_signal)>0).float()
        return out, counter_dummy

    @staticmethod
    def backward(ctx, grad_output, counter):
        decay, = ctx.saved_tensors
        counter = counter + 1
        factor = (1-decay)/(1-decay**int(counter.item()))
        return factor*grad_output, -factor * grad_output, decay * grad_output, torch.zeros([1], device=grad_output.device), counter
    ...
    def __init__(self, architecture, bias=True):
        ...
        for layer, params in self.architecture.items():
            ...
            if params[2]:
                ...
                self.layers[layer+'_synapse'] = params[2](n, params[1].in_size, bias=self.layers[layer].do_bias)
                if self.layers[layer].do_bias:
                    with torch.no_grad():
